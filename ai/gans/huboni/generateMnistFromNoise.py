...
# Generator
def generator(x):
    h1 = tf.nn.relu(tf.add(tf.matmul(x,weights['gen_hidden1']),biases['gen_hidden1']))
    g1 = tf.nn.sigmoid(tf.add(tf.matmul(h1,weights['gen_out']),biases['gen_out']))
    out_layer = g1
    return out_layer
...
def discriminator(x):
    # TO DO
    h2 = tf.nn.relu(tf.add(tf.matmul(x,weights['disc_hidden1']),biases['disc_hidden1']))
    d2 = tf.nn.sigmoid(tf.add(tf.matmul(h2,weights['disc_out']),biases['disc_out']))
    out_layer = d2
    return out_layer
...
with tf.Session() as sess:
    ...
    plt.savefig('./MnistFromNoise.png')
    f.show()
    plt.draw()
